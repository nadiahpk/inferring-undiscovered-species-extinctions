# the same as sensitivity_species_deletion.py,
# except that I assume that we *don't* know what UT is at the end, that we assume UT=0 (see NOTE below)
...
for subset_size in subset_sizes:
    ...
    for sample in range(samples_per_subset):
        ...
        '''
        # NOTE: assume we don't know U_T > 0, we assume U_T = 0
        U_T = 0
    ...
    f = open(dir_results + 'sensitivity_spp_deletion_UT0_' + str(subset_size) + '.pkl', 'wb')
...
f = open(dir_results + 'sensitivity_spp_deletion_UT0_all.pkl', 'wb')
...
# plt.show()
plt.tight_layout()
plt.savefig(dir_results + 'sensitivity_species_deletion_UT0.pdf')
plt.close()
